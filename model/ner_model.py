...
class NERModel(BaseModel):
    ...
    def add_loss_op(self):
        ...
        if self.config.use_crf:
            ...
            self.sum_loss = tf.reduce_sum(losses)
    ...
    def predict_batch(self, words):
        ...
        sum_loss = self.sess.run(self.sum_loss, feed_dict=fd)

        return labels_pred, sequence_lengths, labels_probility, sum_loss
    ...
    def run_evaluate(self, test, vocab_tags = None):
        ...
        correct_preds, total_correct, total_preds, total_loss = 0., 0., 0., 0.
        for words, labels, masks in minibatches(test, self.config.batch_size):
            labels_pred, sequence_lengths, pred_prob, sum_loss = self.predict_batch(words)
            total_loss += sum_loss
        ...
        return {"acc": 100*acc, "f1": acc*100, "total_loss" : total_loss}
